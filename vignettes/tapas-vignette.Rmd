...
library(remotes)
remotes::insttrain_github('avalcarcel9/rtapas')
```
...
```{r}
ids = paste0('subject_', 1:length(train_gold_standard_masks))
```
...
```{r}
ids = paste0('subject_', (10 + 1:length(test_gold_standard_masks)))
```
...
```
#### SHOW THE TAPAS MASKS AND THEN
#### CALCULATE DSC FOR EACH GROUP AND TAPAS MASK
